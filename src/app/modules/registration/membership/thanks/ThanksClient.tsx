  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        ...
        else { setLoading(false); setError("We’re still finalizing your membership. Please refresh in a moment."); }
      } catch (e: any) { setLoading(false); setError(e?.message || "Lookup failed"); }
    }
    if (sessionId) tick(); else { setLoading(false); setError("Missing session_id."); }
    return () => { stop = true; };
  }, [sessionId]);
